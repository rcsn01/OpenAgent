...
export function formatServerError(error: unknown, translate?: Translator, fallback?: string) {
  ...
  const objectMessage = parseObjectMessage(unwrapped)
...
function parseReadableProviderModelNotFoundError(errorInput: ProviderModelNotFoundError, translator?: Translator) {
  ...
  const tail = tr(translator, "error.chain.checkConfig", "Check your config (openagent.json) provider/model names")
  if (list.length) {
