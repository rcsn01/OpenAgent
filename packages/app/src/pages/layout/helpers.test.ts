...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openagent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openagent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openagent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openagent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openagent://open-project")).toBeUndefined()
    expect(parseDeepLink("openagent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openagent://open-project?directory=/a",
      "openagent://other?directory=/b",
      "openagent://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openagent://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openagent://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openagent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openagent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openagent://new-session?directory=/a",
      "openagent://open-project?directory=/b",
      "openagent://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENAGENT__: {
        deepLinks: ["openagent://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENAGENT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openagent://open-project?directory=/a"])
