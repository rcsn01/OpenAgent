...
export function sampledChecksum(content: string, limit = 500_000): string | undefined {
  ...
  const size = 4096
  const points = [0, content.length * 0.25, content.length * 0.5, content.length * 0.75, content.length - size]
  const hashes = points
    .map((point) => {
      const start = Math.max(0, Math.min(content.length - size, Math.floor(point) - Math.floor(size / 2)))
