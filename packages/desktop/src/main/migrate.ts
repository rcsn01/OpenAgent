...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.openagent.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openagent.settings.dat` is special: it maps to the `openagent.settings` store
...
function migrateFile(datPath: string, filename: string) {
  ...
  // openagent.settings.dat → the electron settings store ("openagent.settings").
  ...
  const storeName = filename === "openagent.settings.dat" ? "openagent.settings" : filename
