...
const APP_IDS: Record<string, string> = {
  dev: "ai.openagent.desktop.dev",
  beta: "ai.openagent.desktop.beta",
  prod: "ai.openagent.desktop",
}
const TEST_ONBOARDING = process.env.OPENAGENT_TEST_ONBOARDING === "1"
const SHARED_SERVER_URL = process.env.OPENAGENT_DESKTOP_SERVER_URL?.trim().replace(/\/+$/, "")
...
const main = Effect.gen(function* () {
  ...
  process.env.OPENAGENT_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.openagent.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `openagent-onboarding-${randomUUID()}`)
    rmSync(root, { recursive: true, force: true })
    ;["data", "config", "cache", "state", "desktop", "session"].forEach((dir) =>
      mkdirSync(join(root, dir), { recursive: true }),
    )
    process.env.OPENAGENT_DB = ":memory:"
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("openagent://"))
    if (urls.length) {
  ...
  const needsMigration = ((): boolean => {
    ...
    if (process.env.OPENAGENT_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "openagent", "openagent.db"))
  })()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.OPENAGENT_PORT
    if (fromEnv) {
