...
import "./styles.css"
import { useTheme } from "@openagent-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENAGENT_CHANNEL === "prod" &&
...
const deepLinkEvent = "openagent:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OPENAGENT__ ??= {}
  const pending = window.__OPENAGENT__.deepLinks ?? []
  window.__OPENAGENT__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  return {
    ...
    startSpeechCaptureSession: (config) => window.api.startSpeechCaptureSession(config),
    ...
    beginSpeechCaptureTurn: (sessionId) => window.api.beginSpeechCaptureTurn(sessionId),

    transcribeSpeechCaptureChunk: (input) => window.api.transcribeSpeechCaptureChunk(input),

    transcribeSpeechCaptureTurn: (input) => window.api.transcribeSpeechCaptureTurn(input),
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("openagent.global.dat").getItem("language")
