...
describe("session HttpApi", () => {
  test("serves read routes through Hono bridge", async () => {
    ...
    expect(await json<MessageV2.WithParts>(await app().request(pathFor(SessionPaths.message, { sessionID: parent.id, messageID: message.info.id }), { headers }))).toMatchObject({ info: { id: message.info.id } })
  })
