...
import { createOpenAgent, type ToolPart } from "@openagent-ai/sdk"
...
const openagent = await createOpenAgent({
  port: 0,
})
console.log("✅ OpenAgent server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await openagent.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new OpenAgent session...")
    const { client, server } = openagent
    ...
    console.log("✅ Created OpenAgent session:", createResult.data.id)
  ...
  console.log("📝 Sending to OpenAgent:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OpenAgent response:", JSON.stringify(result, null, 2))
