...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@openagent/acme")).toBe("@openagent/acme")
    expect(Npm.sanitize("@openagent/acme@1.0.0")).toBe("@openagent/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/openagent/acme.git"
    const expected = win ? "acme@git+https_//github.com/openagent/acme.git" : spec
