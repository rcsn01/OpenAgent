...
- Author package entrypoint is `@openagent-ai/plugin/tui`.
...
- Import types from `@openagent-ai/plugin/tui`.
- `@openagent-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running OpenAgent version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openagent` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.openagent` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openagent`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openagent`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openagent` dir or `OPENAGENT_CONFIG_DIR`, OpenAgent installs `@openagent-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openagent-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenAgent's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openagent-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.openagent/themes` area near the plugin config source.
...
- `--pure` / `OPENAGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openagent/plugins/tui-smoke.tsx`
- Local vim plugin: `.openagent/plugins/tui-vim.tsx`
- Local smoke config: `.openagent/tui.json`
- Local smoke theme: `.openagent/plugins/smoke-theme.json`
