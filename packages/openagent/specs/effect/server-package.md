...
- The server still lives in `packages/openagent`.
...
Until enough shared service code lives outside `packages/openagent`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openagent`

It should not import `packages/openagent` services while `packages/openagent`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openagent` implementation details.
