...
Delete `packages/openagent/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openagent/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openagent/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- TUI plugin loader tests in `packages/openagent` if runtime plugin API wiring changed
