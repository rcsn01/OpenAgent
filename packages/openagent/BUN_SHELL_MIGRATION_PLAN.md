...
Replace runtime Bun shell template-tag usage in `packages/openagent/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openagent/src` is removed except:
