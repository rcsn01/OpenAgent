...
const context = Effect.gen(function* () {
  const instance =
    (yield* InstanceRef) ?? (yield* InstanceState.context.pipe(Effect.catchDefect(() => Effect.succeed(undefined))))
  return {
    instance,
    workspaceID: (yield* WorkspaceRef) ?? (yield* InstanceState.workspaceID),
  }
})
