...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for OpenAgent.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal OpenAgent sessions
...
openagent acp --cwd /path/to/project
```
...
```bash
OPENAGENT_ENABLE_QUESTION_TOOL=1 openagent acp
```
...
- **Terminal Integration**: Full terminal support via OpenAgent's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openagent acp
```
...
### Mapping to OpenAgent

ACP sessions map cleanly to OpenAgent's internal session model:
