import type { Hooks, PluginInput } from "@openagent-ai/plugin"
import type { Model } from "@openagent-ai/sdk/v2"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>OpenAgent - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to OpenAgent."
...
async function createModelAccessKey(bearer: string): Promise<ApiKeyInfo> {
  ...
  const res = await fetch(`${DO_API_BASE}/v2/gen-ai/models/api_keys`, {
    ...
    headers: {
      ...
      "User-Agent": `openagent/${InstallationVersion}`,
    },
        ...
        if (bearerValid && stale) {
          ...
          if (result.ok) {
            ...
            await input.client.auth
              ...
              .catch((err: unknown) => log.warn("failed to persist refreshed routers", { error: err }))
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              ...
              instructions:
                "Sign in to DigitalOcean in your browser. OpenAgent will create a Model Access Key named opencode-oauth-* and load your Inference Routers. Re-run /connect to refresh routers later.",
