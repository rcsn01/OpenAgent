...
function managedSkillRoot(root: string, extensionID: string) {
  return path.join(root, ".openagent", "skills", "extensions", extensionID)
}
...
async function selectConfigFile(input: { directory: string; worktree: string }) {
  ...
  const existing = await Filesystem.findUp(["openagent.jsonc", "openagent.json"], input.directory, root)
  const file = existing[0] ?? path.join(root, ".openagent", "openagent.json")
  return { file, root }
}
...
export class Service extends Context.Service<Service, Interface>()("@openagent/Extension") {}
