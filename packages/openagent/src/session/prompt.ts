...
import { AgentAttachment, FileAttachment, ReferenceAttachment, Source } from "@openagent-ai/core/session-prompt"
import { Reference } from "@/reference/reference"
import * as DateTime from "effect/DateTime"
import { eq } from "@/storage/db"
import * as Database from "@/storage/db"
import { SessionTable } from "./session.sql"
import { OpenAgentPiRuntime } from "@/pi/runtime"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const pi = yield* OpenAgentPiRuntime.Service
        ...
        ? [
            {
              ...
              timestamp: firstInfo.time.created,
            },
          ]
        : fromOpenAgentMessages(context)
      const text = yield* pi
        .generateText({
          ...
          messages: msgs,
          prompt:
            "Generate a concise title for this conversation. Output only the title, with no surrounding quotes.",
        })
        .pipe(Effect.orDie)
      const cleaned = text
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
        ...
        ask: (req) =>
          permission
            .ask({
              ...req,
              sessionID: input.session.id,
              tool: { messageID: input.assistant.id, callID: options.toolCallId ?? "" },
      ...
      for (const item of yield* registry.tools({
        ...
        tools.push({
          ...item,
          jsonSchema: ProviderTransform.schema(input.model, ToolJsonSchema.fromTool(item)),
          execute(args, ctx) {
            return Effect.gen(function* () {
              const toolCtx = context(args, { abortSignal: ctx.abort, toolCallId: ctx.callID })
              yield* plugin.trigger(
                "tool.execute.before",
                { tool: item.id, sessionID: toolCtx.sessionID, callID: toolCtx.callID },
                { args },
              )
              const result = yield* item.execute(args, toolCtx)
              const output = {
                ...result,
              ...
              yield* plugin.trigger(
                "tool.execute.after",
                { tool: item.id, sessionID: toolCtx.sessionID, callID: toolCtx.callID, args },
                output,
              )
              return output
            })
        ...
        tools.push({
          ...
          execute(args, ctx) {
            return Effect.gen(function* () {
              ...
              yield* plugin.trigger(
                "tool.execute.before",
                { tool: key, sessionID: toolCtx.sessionID, callID: toolCtx.callID },
                { args },
              )
              const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* Effect.gen(function* () {
                yield* toolCtx.ask({ permission: key, metadata: {}, patterns: ["*"], always: ["*"] })
                return yield* Effect.promise(() =>
                  execute(args, {
                    toolCallId: toolCtx.callID ?? "",
                    abortSignal: toolCtx.abort,
                    messages: [],
                  }),
                )
              }).pipe(
                Effect.withSpan("Tool.execute", {
                  attributes: {
                    ...
                    "message.id": input.assistant.id,
                  },
                }),
              )
              yield* plugin.trigger(
                "tool.execute.after",
                { tool: key, sessionID: toolCtx.sessionID, callID: toolCtx.callID, args },
                result,
              )
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            if (lastUser.format?.type === "json_schema") {
              tools.push(createStructuredOutputTool({
            ...
            if (isLastStep) system.push(MAX_STEPS)
            const result = yield* pi
              .run({
            ...
            const finished = msg.finish && !["tool-calls", "unknown"].includes(msg.finish)
            if (finished && !msg.error) {
              if (format.type === "json_schema") {
                msg.error = new MessageV2.StructuredOutputError({
...
export const defaultLayer = Layer.suspend(() =>
  layer.pipe(
    ...
    Layer.provide(OpenAgentPiRuntime.defaultLayer),
...
type StructuredOutputTool = Tool.Def & {
  inputSchema: { jsonSchema: Record<string, any> }
  toModelOutput(input: { output: { output: string } }): { type: "text"; value: string }
}
