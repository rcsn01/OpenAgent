...
import type { Prompt } from "@openagent-ai/core/session-prompt"
import { ProjectID } from "@/project/schema"
import { SessionEvent } from "@openagent-ai/core/session-event"
import { V2Schema } from "@openagent-ai/core/v2-schema"
...
import { Session as SessionLegacy } from "@/session/session"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const legacy = yield* SessionLegacy.Service
    ...
    const result = Service.of({
      create: Effect.fn("V2Session.create")(function* (input) {
        const info = yield* legacy.create({
        ...
        const row = Database.use((db) => db.select().from(SessionTable).where(eq(SessionTable.id, info.id)).get())
        return fromRow(row ?? ({ ...SessionLegacy.toRow(info), time_archived: info.time.archived ?? null } as typeof SessionTable.$inferSelect))
      }),
      ...
      prompt: Effect.fn("V2Session.prompt")(function* (input) {
        ...
        const agents = input.prompt.agents ?? []
        const parts: SessionPrompt.PromptInput["parts"] = [
          ...(input.prompt.text ? [{ type: "text" as const, text: input.prompt.text }] : []),
          ...files.map((file) => ({
            type: "file" as const,
            url: file.uri,
            mime: file.mime,
          ...
          ...(input.prompt.references ?? []).map((reference) => ({
            type: "text" as const,
            text: reference.problem
              ? `Referenced ${reference.name}: ${reference.problem}`
              : `Referenced ${reference.name}${reference.target ? `/${reference.target}` : ""}.`,
            synthetic: true,
            metadata: { reference },
          })),
        ] as SessionPrompt.PromptInput["parts"]
        const legacyMessage = yield* prompts.prompt({
          sessionID: input.sessionID,
          noReply: input.delivery === "deferred",
          parts: parts.length ? parts : [{ type: "text", text: "" }],
        }).pipe(Effect.orDie)
        if (input.delivery === "deferred")
          yield* prompts.loop({ sessionID: input.sessionID }).pipe(Effect.ignore, Effect.forkDetach)
        return new SessionMessage.User({
          id: SessionMessage.ID.make(input.id ?? legacyMessage.info.id),
          type: "user",
      ...
      compact: Effect.fn("V2Session.compact")(function* (sessionID) {
        ...
        yield* prompts.loop({ sessionID }).pipe(Effect.ignore)
      }),
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(SessionLegacy.defaultLayer),
