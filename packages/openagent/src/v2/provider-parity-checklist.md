...
This tracks legacy provider behavior from `packages/openagent/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/openagent/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.
...
- [x] OpenAgent unauthenticated paid-model filtering.
- [x] OpenAgent public API key fallback.
...
- [x] OpenAgent small model priority. Replaced by cheapest output cost selection.
