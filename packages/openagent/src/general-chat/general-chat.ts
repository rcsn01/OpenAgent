...
export class Service extends Context.Service<Service, Interface>()("@openagent/GeneralChat") {}
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("GeneralChat.create")(function* () {
      ...
      if (config && hasConfigValue(config)) {
        yield* fs
          .writeWithDirs(pathSvc.join(directory, "openagent.json"), JSON.stringify(config, null, 2))
          .pipe(Effect.orDie)
      }
