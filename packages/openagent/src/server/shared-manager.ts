...
function envUrl() {
  const raw = process.env.OPENAGENT_SERVER_URL || process.env.OPENAGENT_SERVER_URL
  if (!raw?.trim()) return
...
function credentials(metadata?: Pick<Metadata, "username" | "password">) {
  ...
  return {
    username: process.env.OPENAGENT_SERVER_USERNAME,
    password: process.env.OPENAGENT_SERVER_PASSWORD,
  }
}
...
async function spawnServer() {
  ...
  const username = process.env.OPENAGENT_SERVER_USERNAME || "openagent"
  const password = process.env.OPENAGENT_SERVER_PASSWORD || randomUUID()
  ...
  const child = spawn(command.cmd, command.args, {
    ...
    env: {
      ...process.env,
      OPENAGENT_SERVER_USERNAME: username,
      OPENAGENT_SERVER_PASSWORD: password,
...
export async function ensure(): Promise<Metadata> {
  ...
  if (forced) {
    return {
      pid: 0,
      url: forced,
      username: process.env.OPENAGENT_SERVER_USERNAME || "openagent",
      password: process.env.OPENAGENT_SERVER_PASSWORD || "",
      dbPath: Database.getPath(),
      startedAt: new Date().toISOString(),
      version: InstallationVersion,
