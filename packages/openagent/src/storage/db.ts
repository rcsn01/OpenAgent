...
declare const OPENAGENT_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: ChannelDbFlags = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "openagent.db")
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  return path.join(Global.Path.data, `openagent-${safe}.db`)
}

export const getPath = (flags?: ChannelDbFlags) => {
  if (Flag.OPENAGENT_DB) {
    if (Flag.OPENAGENT_DB === ":memory:" || path.isAbsolute(Flag.OPENAGENT_DB)) return Flag.OPENAGENT_DB
    return path.join(Global.Path.data, Flag.OPENAGENT_DB)
  }
...
export const Client = Object.assign(
  (flags?: ChannelDbFlags): Client => {
    ...
    const entries =
      typeof OPENAGENT_MIGRATIONS !== "undefined"
        ? OPENAGENT_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof OPENAGENT_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
      if (Flag.OPENAGENT_SKIP_MIGRATIONS) {
