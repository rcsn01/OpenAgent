...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@openagent-ai/core/schema"
...
// Uses the shared `DeepMutable` from `@openagent-ai/core/schema`. See the definition
...
function globalConfigFile() {
  const candidates = ["openagent.jsonc", "openagent.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openagent.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openagent.jsonc")))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/openagent` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/openagent`))
            if (!response.ok) {
            ...
            const remote = yield* Effect.promise(() =>
              substituteWellKnownRemoteConfig({
                ...
                dir: url,
                source: `${url}/.well-known/openagent`,
              }),
            )
            ...
            const source = `${url}/.well-known/openagent`
        ...
        if (Flag.OPENAGENT_CONFIG) {
          yield* merge(Flag.OPENAGENT_CONFIG, yield* loadFile(Flag.OPENAGENT_CONFIG))
        ...
        for (const dir of directories) {
          if (dir.endsWith(".openagent") || dir === Flag.OPENAGENT_CONFIG_DIR || dir === ConfigPaths.chatDirectory()) {
