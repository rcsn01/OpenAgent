...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openagent.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openagent/ subdirectory too)
  const candidates = [path.join(baseDir, "openagent.json"), path.join(baseDir, "openagent.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openagent", "openagent.json"), path.join(baseDir, ".openagent", "openagent.jsonc"))
  }
