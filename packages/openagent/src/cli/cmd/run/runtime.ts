...
import { createOpenAgentClient } from "@openagent-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openagent.mode": input.resolveSession ? "local" : "attach",
      "openagent.initial_input": !!input.initialInput,
      "openagent.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "openagent.directory": ctx.directory,
        "openagent.resume": ctx.resume === true,
        "openagent.agent.name": state.agent,
        "openagent.model.provider": state.model?.providerID,
        "openagent.model.id": state.model?.modelID,
        "openagent.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "openagent.model.provider": model.providerID,
            "openagent.model.id": model.modelID,
            "openagent.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "openagent.model.id": state.model?.modelID,
                "openagent.model.variant": state.activeVariant,
                "openagent.prompt.chars": prompt.text.length,
                "openagent.prompt.parts": prompt.parts.length,
                "openagent.prompt.include_files": includeFiles,
                "openagent.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openagent.directory": input.directory,
      "openagent.initial_input": !!input.initialInput,
      "openagent.demo": input.demo,
    },
    async () => {
