...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "openagent.agent.name": input.agent,
      "openagent.directory": input.directory,
      "openagent.first": input.first,
      "openagent.model.provider": input.model?.providerID,
      "openagent.model.id": input.model?.modelID,
      "openagent.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "openagent.show_exit": next.showExit,
