...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openagent",
      pnpm: "pnpm uninstall -g openagent",
      bun: "bun remove -g openagent",
      yarn: "yarn global remove openagent",
      brew: "brew uninstall openagent",
      choco: "choco uninstall openagent",
      scoop: "scoop uninstall openagent",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openagent", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".openagent") || binDir.includes(".openagent")) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes("# openagent") ||
      content.includes("# openagent") ||
      content.includes(".openagent/bin") ||
      content.includes(".openagent/bin")
    ) {
      return file
    }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# openagent" || trimmed === "# openagent") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openagent/bin") || trimmed.includes(".openagent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".openagent/bin") || trimmed.includes(".openagent/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".openagent") || trimmed.includes(".openagent")))
    ) {
      continue
    }
