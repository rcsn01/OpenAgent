// CLI entry point for `openagent run`.
//
// Handles three modes:
...
//      OpenAgent server and runs interactive mode against it.
//
...
import { createOpenAgentClient, type OpenAgentClient, type ToolPart } from "@openagent-ai/sdk/v2"
...
type ModelInput = Parameters<OpenAgentClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
        ...
        !localMode || args.attach || forcedUrl
          ? args.attach || forcedUrl
            ? {
                ...
                username: args.username ?? process.env.OPENAGENT_SERVER_USERNAME ?? "openagent",
                password: args.password ?? process.env.OPENAGENT_SERVER_PASSWORD ?? "",
              }
      ...
      async function session(sdk: OpenAgentClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OpenAgentClient, sessionID: string) {
      ...
      async function current(sdk: OpenAgentClient): Promise<string> {
        if (!serverUrl) {
      ...
      async function attachAgent(sdk: OpenAgentClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OpenAgentClient) {
        if (!args.agent) return undefined
        if (serverUrl) {
      ...
      async function execute(sdk: OpenAgentClient) {
        ...
        async function loop(client: OpenAgentClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOpenAgentClient({
        baseUrl: "http://openagent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
