...
function localOAuthEnvironmentStatus(mcpName: string, mcp: ConfigMCP.Local): Status | undefined {
  ...
  return {
    ...
    error: `${missing.join(", ")} is not available to the OpenAgent app process. Set it before launching OpenAgent Desktop, then reconnect ${mcpName}.`,
  }
}
  ...
  Effect.gen(function* () {
    ...
    const connectLocalHTTPProcess = Effect.fn("MCP.connectLocalHTTPProcess")(function* (
      ...
      const childEnv: Record<string, string> = {
        ...
        ...(command === "openagent" ? { BUN_BE_BUN: "1" } : {}),
        ...mcp.environment,
        ...
        const result = yield* connectTransport(transport, connectTimeout).pipe(
          ...
          Effect.catch((error) => {
            ...
            if (isAuthError) {
              ...
              return oauthWarning(
                key,
                `Server "${key}" requires authentication. Run: openagent mcp auth ${key}`,
                lastStatus,
              ).pipe(Effect.as(undefined))
            }
              ...
              return local.cleanup.pipe(
                Effect.andThen(
                  lastError.message.includes("registration") || lastError.message.includes("client_id")
                    ...
                    : oauthWarning(key, `Server "${key}" requires authentication. Run: openagent mcp auth ${key}`, {
                        status: "needs_auth",
                      }),
                ),
