...
export class Service extends Context.Service<Service, Interface>()("@openagent/IntegrationAuth") {}
...
function oauthUrl(input: StartInput & { state: string; connectionID: string; accountID: string }) {
  if (input.provider === "composio") {
    ...
    if (!clientID) return `openagent://integrations/composio/setup?state=${encodeURIComponent(input.state)}`
  ...
  return `openagent://integrations/${input.provider}/setup?state=${encodeURIComponent(input.state)}`
}
