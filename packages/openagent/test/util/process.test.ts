...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OPENAGENT_TEST ?? "")'), {
      env: {
        OPENAGENT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OPENAGENT_TEST_SHELL"], {
      shell: true,
      env: {
        OPENAGENT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OPENAGENT_TEST_SHELL=ok")
  })
