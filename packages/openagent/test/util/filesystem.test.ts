...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openagent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openagent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openagent.json", "openagent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "openagent.json"),
        path.join(tmp.path, "openagent.jsonc"),
        path.join(project, "openagent.json"),
        path.join(project, "openagent.jsonc"),
      ])
    })
  })
