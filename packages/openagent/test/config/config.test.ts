...
const managedConfigDir = process.env.OPENAGENT_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "openagent.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openagent.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "openagent.jsonc"))
    expect(content).toContain('"$schema": "https://openagent.ai/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("does not create global config when OPENAGENT_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.OPENAGENT_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.OPENAGENT_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "openagent.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.OPENAGENT_CONFIG_DIR
    else process.env.OPENAGENT_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "openagent.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "openagent.jsonc")
  ...
  try {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openagent.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OPENAGENT_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:OPENAGENT_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["OPENAGENT_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENAGENT_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openagent.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .openagent directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openagent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".openagent", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .openagent/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openagent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openagent/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openagent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openagent/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openagent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only OPENAGENT_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENAGENT_CONFIG_DIR
  process.env.OPENAGENT_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENAGENT_CONFIG_DIR
    else process.env.OPENAGENT_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENAGENT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "openagent.json"),
        JSON.stringify({ $schema: "https://openagent.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openagent config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".openagent")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .openagent config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "openagent.json"),
        JSON.stringify({
          $schema: "https://openagent.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openagent config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "openagent.json"),
        JSON.stringify({
          $schema: "https://openagent.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".openagent")
      ...
      await Filesystem.write(
        path.join(local, "openagent.json"),
        JSON.stringify({
          $schema: "https://openagent.ai/config.json",
...
// Note: preload.ts sets OPENAGENT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openagent config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openagent directory config enables it
      const opencodeDir = path.join(dir, ".openagent")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "openagent.json"),
    ...
    if (urlStr.includes(".well-known/opencode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/openagent.json",
              headers: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(remoteFetchedUrl).toBe("https://config.example.com/openagent.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "openagent.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.openagent/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.openagent/plugin/demo.ts", "file:///project/.openagent/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.openagent/plugin/demo.ts"])
  })
...
describe("OPENAGENT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openagent/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openagent directory with a command
          const opencodeDir = path.join(dir, ".openagent", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          // Project .openagent should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENAGENT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENAGENT_CONFIG_DIR"]
      process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENAGENT_CONFIG_DIR"]
      } else {
        process.env["OPENAGENT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENAGENT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENAGENT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENAGENT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENAGENT_CONFIG_DIR"] = configDirTmp.path

      await provideTestInstance({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from OPENAGENT_CONFIG_DIR, not project
...
describe("OPENAGENT_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENAGENT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENAGENT_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENAGENT_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://openagent.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENAGENT_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENAGENT_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENAGENT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENAGENT_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "OpenAgent Managed",
          PayloadIdentifier: "ai.openagent.managed.test",
          PayloadType: "ai.openagent.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://openagent.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://openagent.ai/config.json")
})
