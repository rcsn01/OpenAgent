...
9. Run the focused test file and `bun typecheck` from `packages/openagent`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep" -- packages/openagent/test
```
