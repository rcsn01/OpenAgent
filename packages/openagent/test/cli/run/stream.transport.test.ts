...
import { OpenAgentClient, type GlobalEvent } from "@openagent-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OpenAgentClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OpenAgentClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OpenAgentClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OpenAgentClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OpenAgentClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: OpenAgentClient["event"]["subscribe"]
    globalEvent?: OpenAgentClient["global"]["event"]
    promptAsync?: OpenAgentClient["session"]["promptAsync"]
    status?: OpenAgentClient["session"]["status"]
    messages?: OpenAgentClient["session"]["messages"]
    children?: OpenAgentClient["session"]["children"]
    permissions?: OpenAgentClient["permission"]["list"]
    questions?: OpenAgentClient["question"]["list"]
  } = {},
) {
  const client = new OpenAgentClient()

  const subscribe: OpenAgentClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OpenAgentClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OpenAgentClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OpenAgentClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OpenAgentClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OpenAgentClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OpenAgentClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OpenAgentClient["question"]["list"] = input.questions ?? (() => ok([]))
