...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@openagent-ai/plugin": "0.0.0" } } } }),
  )
}
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".openagent")
  ...
  const first = await provideTestInstance({
    directory: tmp.path,
    fn: async () =>
  ...
  const second = await provideTestInstance({
    directory: tmp.path,
    fn: async () => list(),
  })
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".openagent", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("opencode loader keeps paid models when config apiKey is present", async () => {
  ...
  const none = await provideTestInstance({
    directory: base.path,
  ...
  const keyedCount = await provideTestInstance({
