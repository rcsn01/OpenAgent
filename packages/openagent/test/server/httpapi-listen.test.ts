...
const original = {
  ...
  envPassword: process.env.OPENAGENT_SERVER_PASSWORD,
  envUsername: process.env.OPENAGENT_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OPENAGENT_SERVER_PASSWORD
  else process.env.OPENAGENT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OPENAGENT_SERVER_USERNAME
  else process.env.OPENAGENT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OPENAGENT_SERVER_PASSWORD = auth.password
  Flag.OPENAGENT_SERVER_USERNAME = auth.username
  process.env.OPENAGENT_SERVER_PASSWORD = auth.password
  process.env.OPENAGENT_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.OPENAGENT_SERVER_USERNAME = auth.username
  delete process.env.OPENAGENT_SERVER_PASSWORD
  process.env.OPENAGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-openagent-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-openagent-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-openagent-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-openagent-ticket": "1" },
      })
