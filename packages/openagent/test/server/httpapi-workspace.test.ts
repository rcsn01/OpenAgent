...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-openagent-directory", directory)
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-openagent-workspace": "internal",
          },
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openagent-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openagent-workspace")
      } finally {
