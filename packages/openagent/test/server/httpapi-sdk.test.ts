...
type Sdk = ReturnType<typeof createOpenAgentClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.OPENAGENT_SERVER_PASSWORD = input?.password
  Flag.OPENAGENT_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".openagent", "skills", "project-rest-skill", "SKILL.md"),
      `---
