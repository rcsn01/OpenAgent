    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-openagent-directory": "/secret/path",
      ...
      expect(forwarded?.headers["x-openagent-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-openagent-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-openagent-directory", headerDir),
        HttpClient.execute,
      )
