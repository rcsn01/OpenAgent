...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      OPENAGENT_EXPERIMENTAL_WORKSPACES: Flag.OPENAGENT_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.OPENAGENT_EXPERIMENTAL_WORKSPACES = original.OPENAGENT_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
