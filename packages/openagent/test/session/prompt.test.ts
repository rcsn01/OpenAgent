...
import { TestPiRuntime } from "../lib/pi-runtime"
...
const blockingProcessor = Layer.succeed(
  ...
  OpenAgentPiRuntime.Service.of({
    run: () => Effect.sync(() => processorCreateStarted.shift()?.()).pipe(Effect.andThen(Effect.never)),
...
function makeHttp(input?: { processor?: "blocking" }) {
  ...
  const proc =
    input?.processor === "blocking"
      ? blockingProcessor
      : TestPiRuntime.layer.pipe(Layer.provideMerge(llm), Layer.provide(summary), Layer.provideMerge(deps))
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<Config.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://openagent.ai/config.json", ...config }),
  )
})
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* llm.text("packages/openagent/src/session/prompt.ts")

      const result = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        parts: [{ type: "text", text: "Where is SessionPrompt?" }],
      })

      expect(result.info.role).toBe("assistant")
      expect(result.parts.some((part) => part.type === "text" && part.text.includes("prompt.ts"))).toBe(true)
