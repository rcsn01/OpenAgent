...
describe("structured-output.createStructuredOutputTool", () => {
  ...
  test("execute calls onSuccess with valid args", async () => {
    ...
    const result = await Effect.runPromise(tool.execute!(testArgs, toolContext()))
  ...
  test("execute handles nested objects", async () => {
    ...
    const validResult = await Effect.runPromise(
      tool.execute!({ user: { name: "John", email: "john@test.com" } }, toolContext()),
    )
  ...
  test("execute handles arrays", async () => {
    ...
    const validResult = await Effect.runPromise(tool.execute!({ tags: ["a", "b", "c"] }, toolContext()))
