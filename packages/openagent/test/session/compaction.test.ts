...
import type { RunInput } from "@/pi/runtime"
...
namespace LLM {
  export type StreamInput = Partial<RunInput> & { messages: unknown[] }
  export type Event =
    | { type: "start" }
    ...
    | { type: "finish-step"; finishReason: string; rawFinishReason: string; response: unknown; providerMetadata: unknown; usage: unknown }
    | { type: "finish"; finishReason: string; rawFinishReason: string; totalUsage: unknown }
}

function piLayer(result: "continue" | "compact" = "continue") {
  return Layer.succeed(
    OpenAgentPiRuntime.Service,
    OpenAgentPiRuntime.Service.of({
      run: (input) =>
        Effect.sync(() => {
          input.assistantMessage.finish = result === "compact" ? "unknown" : "stop"
          input.assistantMessage.time.completed = Date.now()
          return result
        }),
      generateText: () => Effect.succeed("Generated title"),
      cancel: () => Effect.void,
      wait: () => Effect.void,
    }),
  )
        ...
        run: (input) =>
          Effect.gen(function* () {
            ...
            yield* stream({
                ...
                system: input.system,
                model: input.model,
                messages: fromOpenAgentMessages(input.messages, input.transcriptOptions) as never,
              })
              .pipe(
                ...
                Effect.catch((error: unknown) => {
                  if (APICallError.isInstance(error) && error.isRetryable) {
                    return status
                      ...
                      .pipe(Effect.andThen(Effect.sleep("10 seconds")))
                  }
...
type CompactionProcessOptions = {
  ...
  llm?: Layer.Layer<OpenAgentPiRuntime.Service, never, any>
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  ...
  const pi = options?.llm ?? piLayer(options?.result ?? "continue")
  return Layer.mergeAll(SessionCompaction.layer.pipe(Layer.provide(pi)), pi, bus, status).pipe(
...
function llm() {
  ...
  return {
    ...
    layer: piFromStreamLayer((input) => stream(input).pipe(Stream.mapEffect((event) => Effect.succeed(event)))),
  }
}
