...
namespace TestLLMServer {
  export interface Service {
    ...
    readonly next: (body: Record<string, unknown>) => Effect.Effect<Item>
  }
}
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const queue = (...input: (Item | Reply)[]) => {
        state.list = [...state.list, ...input.map((value) => ({ item: item(value) }))]
      }

      const queueMatch = (match: Match, ...input: (Item | Reply)[]) => {
        state.list = [...state.list, ...input.map((value) => ({ item: item(value), match }))]
      }

      const notify = Effect.fnUntraced(function* () {
        const ready = state.waits.filter((item) => state.hits.length >= item.count)
        if (!ready.length) return
        state.waits = state.waits.filter((item) => state.hits.length < item.count)
        yield* Effect.forEach(ready, (item) => Deferred.succeed(item.ready, void 0))
      })

      const pull = (hit: Hit) => {
        const index = state.list.findIndex((entry) => !entry.match || entry.match(hit))
        if (index === -1) return
        const first = state.list[index]
      ...
      return TestLLMServer.of({
        ...
        reset: Effect.sync(resetState),
        hits: Effect.sync(() => [...state.hits]),
        calls: Effect.sync(() => state.hits.length),
        wait: Effect.fn("TestLLMServer.wait")(function* (count: number) {
          if (state.hits.length >= count) return
          const ready = yield* Deferred.make<void>()
          state.waits = [...state.waits, { count, ready }]
          yield* Deferred.await(ready)
        }),
        inputs: Effect.sync(() => state.hits.map((hit) => hit.body)),
        pending: Effect.sync(() => state.list.length),
        misses: Effect.sync(() => [...state.misses]),
        next: Effect.fn("TestLLMServer.next")(function* (body: Record<string, unknown>) {
          const current = hit("/v1/chat/completions", body)
          if (isTitleRequest(body)) {
            state.hits = [...state.hits, current]
            yield* notify()
            return { type: "sse", head: [role()], tail: [textLine("E2E Title"), finishLine("stop")] }
          }
          const found = pull(current)
          state.hits = [...state.hits, current]
          yield* notify()
          return found ?? ({ type: "sse", head: [role()], tail: [textLine("ok"), finishLine("stop")] } satisfies Sse)
        }),
      })
    }),
  )
