...
it.instance("plan agent denies edits except .openagent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openagent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openagent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENAGENT_TEST_HOME
      process.env.OPENAGENT_TEST_HOME = test.directory
