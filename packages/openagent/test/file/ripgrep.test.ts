...
describe("file.ripgrep", () => {
    ...
    Effect.gen(function* () {
      const dir = yield* tmpdir((dir) =>
        Effect.gen(function* () {
          ...
          yield* mkdir(path.join(dir, ".openagent"))
          yield* write(path.join(dir, ".openagent", "thing.json"), "{}")
        }),
      )

      const files = yield* collectFiles({ cwd: dir })
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".openagent", "thing.json"))).toBe(true)
    }),
  )
  ...
  it.live("hidden false excludes hidden", () =>
    Effect.gen(function* () {
      ...
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".openagent", "thing.json"))).toBe(false)
    }),
  )
