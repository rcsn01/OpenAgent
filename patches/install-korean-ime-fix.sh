...
PROMPT_FILE="$OPENCODE_SRC/external opencode runtime/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$OPENCODE_SRC/external opencode runtime"
...
BUILT_BINARY="$OPENCODE_SRC/external opencode runtime/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$OPENCODE_SRC/external opencode runtime/dist" -name "opencode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $OPENCODE_SRC/external opencode runtime && bun run build --single"
  exit 1
fi
