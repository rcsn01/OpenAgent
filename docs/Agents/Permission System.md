...
The `evaluate` function in `external opencode runtime/src/permission/evaluate.ts` checks a permission request against all applicable rulesets in priority order. The ruleset order is:
...
| `assistant` | `primary` | Provider prompt + `assistant.txt` + external orchestration routing guidance | General-purpose non-coding agent — all tools allowed; `question`, `plan_enter` allowed; **`task` allowed**; **background task/graph, `send_message`, and `composio` allowed** |
...
Three layers of registry gating control which agents receive which tools (`external opencode runtime/src/tool/registry.ts`):
...
Caller-aware spawn filtering in `external opencode runtime/src/agent/spawnable.ts` keeps OpenAgent feature specialists assistant-only:
...
**Denied for:** `plan`, `explore`, `general`, and all 6 external orchestration specialists.
...
**Denied for:** `build`, `plan`, `explore`, `general`, all 6 external orchestration specialists, and blocked coordinator names (`orchestrator`).
...
`send_message` is additionally gated by configured communication flows (`external opencode runtime/src/agent/communication.ts`). The tool only appears if at least one configured recipient exists for that agent+mode combination.
...
The runtime refuses to spawn these names as subagents (`external opencode runtime/src/agent/spawnable.ts`):
