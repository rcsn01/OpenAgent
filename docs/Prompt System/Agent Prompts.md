...
| `slides-agent` | `subagent` | `slides_plan`, `slides_modify`, `slides`, `slides_theme`, `slide_screenshot`, `slide_overflow_check` | `send_message`, `transfer`; use plan/modify before export so decks are not title-only; `slides` writes the HTML/CSS project and exports PPTX |
...
`general` is a generic subagent (no specialist tool) and is only a `send_message` recipient — not a `transfer` recipient.
