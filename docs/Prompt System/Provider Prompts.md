...
The base prompt is selected at runtime in `packages/openagent/src/session/system.ts` based on the active model's ID string:
...
The provider prompt is used when the active agent does **not** define its own `prompt` field. The deciding line is in `packages/openagent/src/session/llm.ts`:
