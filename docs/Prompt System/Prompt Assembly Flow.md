...
In `packages/openagent/src/session/llm.ts`, the system prompt is assembled like this:
...
Then the system is further enriched by `packages/openagent/src/session/prompt.ts`:
